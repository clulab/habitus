...
class TestEntitiesNorms extends Test {
  ...
  // Here, we only test etities that have norms (numeric entities)
  ...
  val variableTests: Array[VariableTest] = Array(
    ...
    VariableTest(
      failingTest,
      "Fix2",
      // FIXME; 9.2 t ha-1 getting missed; should be fixed after the next processors release
      ...
      Seq(
        "Quantity" -> Seq(
          ("between 8.8 t ha-1 and 9.2 t ha-1", "8.8 -- 9.2 t/ha") // note: 1 t/ha is increase, not absolute quantity
        )
      )
    ),
    VariableTest(
    ...
    VariableTest(
      ...
      Seq(
        "Quantity" -> Seq(
          ("2700-2800 kg ha-1", "2700.0 -- 2800.0 kg/ha"),
