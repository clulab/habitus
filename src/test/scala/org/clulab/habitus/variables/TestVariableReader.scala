...
class TestVariableReader extends Test {
  ...
  VariableTest(
    "sent_20", "One of the most important farming input is mineral fertilizer",
    "recognize fertilizer [attached to be verb]",
    Seq(("input", Seq(("mineral", ""))))
  ).test()

  VariableTest(
    "sent_20_1", "Fertilizer nitrogen (N) has been applied at two or more levels",
    "recognize fertilizer [attached to var]",
    Seq(("Fertilizer", Seq(("nitrogen", ""))))
  ).test()

  VariableTest(
    "sent_20_2", "In fact, use of fertilizer P has declined steadily since 1995",
    "recognize fertilizer [attached to variable label]",
    Seq(("fertilizer", Seq(("P", ""))))
  ).test()

  VariableTest(
    "sent_20_3", "The amount of fertilizer N required was averaged at 52 kg ha-1",
    "recognize fertilizer [attached to variable]",
    Seq(("fertilizer", Seq(("N", ""))))
  ).test()

  VariableTest(
    "sent_20_4", "The most widely used solid inorganic fertilizers are urea, diammonium phosphate and potassium chloride",
    "recognize fertilizer [attached to be verb]",
  ...
  VariableTest(
    "sent_20_5_2", "The nitrogenous chemical fertilizers are urea, calcium, ammonium nitrate, ammonium sulfate, basic calcium nitrate, calcium cyanamide",
    "recognize fertilizer [attached to be verb]",
    Seq(("fertilizers", Seq(("urea", ""), ("calcium", ""), ("ammonium nitrate", ""), ("ammonium sulfate", ""), ("basic calcium nitrate", ""), ("calcium cyanamide", ""))))
  ).test()

  VariableTest(
    "sent_20_6", "Total fertilizer usage was ammonium poly-phosphate on average 152 kg ha-1 in the 1999 and 2000WS",
    "recognize fertilizer [attached to be verb]",
    Seq(("fertilizer usage", Seq(("ammonium poly-phosphate", ""))))
  ).test()

  VariableTest(
    "sent_20_7", "However, the most unvalable fertilizer was diammonium-phosphate",
    "recognize fertilizer [attached to be verb]",
    Seq(("fertilizer", Seq(("diammonium-phosphate", ""))))
  ).test()

  VariableTest(
    "sent_20_8", "Phosphorus, potassium and NPK are important inorganic fertilizers.",
  ...
  VariableTest(
    "sent_20_9", "Some organic fertilizers include nitrogen, phosphorus, and potassium as the three most important elements for plant nutrition",
    "recognize comma separated fertilizers types",
    Seq(
