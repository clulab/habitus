...
//  val articleFileName = args.lift(1).getOrElse("./scraper/articlecorpus-illegal-mining.txt")
  val articleFileName = args.lift(1).getOrElse("./scraper/corpora/multi2/livestock/articlecorpus.txt")

  val baseDirName = args.lift(1).getOrElse("../corpora/multi2/livestock/indexes")
