...
object ArticleScraperApp extends App {
  val term = "test"
  val corpusFileName = args.lift(0).getOrElse(s"./scraper/corpora/mail/$term/articlecorpus.txt")
  val baseDirName = args.lift(1).getOrElse(s"../corpora/mail/$term/articles")
