...
object SearchScraperApp extends App {
  val corpusFileName = args.lift(0).getOrElse("./scraper/corpora/uganda/uganda mining/searchcorpus.txt")
  val articleFileName = args.lift(1).getOrElse("./scraper/corpora/uganda/uganda mining/indexcorpus.txt")
  val baseDirName = args.lift(2).getOrElse("../corpora/uganda/uganda mining/searches")
