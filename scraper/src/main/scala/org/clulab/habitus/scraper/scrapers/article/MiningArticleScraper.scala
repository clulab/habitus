...
import org.clulab.habitus.scraper.domains.MiningDomain
import org.clulab.habitus.scraper.scrapes.ArticleScrape
import org.json4s.jackson.JsonMethods
import org.json4s.{DefaultFormats, JArray, JObject}

class MiningArticleScraper extends PageArticleScraper(MiningDomain) {
  ...
  def scrape(browser: Browser, page: Page, html: String): ArticleScrape = {
    ...
    val bylineOpt = personOpt
      .map { person => (person \ "name").extract[String] }
      .orElse {
        val lines = doc >> elementList("div.single div.post-meta > a")

        lines.headOption.map(_.text.trim)
      }
